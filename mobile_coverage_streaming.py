...
try:
    ...
    # Apply watermarking on the new timestamp column
    data_frame_with_watermark = data_frame_with_partition_hour.withWatermark("timestamp", "10 minutes")
    
    # 1. Analyze the average signal strength by location and operator
    ...
    # 2. Track the GPS precision and the number of satellites connected by different providers.
    gps_precision_by_provider_df = data_frame_with_watermark.groupBy(
        window(col("timestamp"), "2 minutes"), "provider"
    ).agg(
    ...
    # 3. Status Analysis:Count occurrences of different statuses (like 'active', 'inactive', etc.) in each postal code. 
    ...
    s3_path = "s3://ulabs-streaming-aggregates-v1/aggregations/"
    s3_path_checkpoint = "s3://ulabs-streaming-aggregates-v1/checkpoints/"
